...
async def create_single_user(request):
    ...
    try:
        ...
        async def generate_qr_and_send_email():
            ...
            pdf_url = await generate_svg([{ "email": email, "userId": new_user.uid, "qr_code_file": qr_code_file }], request.customer)
            await send_email_with_qr_code(pdf_url)
